...
SELECT
    pk_rgm,
    nome_aluno
FROM faculdade_nova_horizonte.tb_alunos;

INSERT INTO dim_curso (nk_id_curso, nome_curso)
SELECT
    pk_id_curso,
    nome_curso
FROM faculdade_nova_horizonte.tb_cursos;

INSERT INTO dim_unidade (nk_id_unidade, nome_unidade)
SELECT
    pk_id_unidade,
    nome_unidade
FROM faculdade_nova_horizonte.tb_unidades;
...
INNER JOIN faculdade_nova_horizonte.tb_contratos_educacionais ce
    ON p.fk_id_contrato = ce.pk_id_contrato

INNER JOIN faculdade_nova_horizonte.tb_alunos a
    ON ce.fk_rgm = a.pk_rgm

INNER JOIN faculdade_nova_horizonte.tb_cursos c
    ON a.fk_id_curso = c.pk_id_curso

INNER JOIN dim_aluno da
    ON da.nk_rgm = a.pk_rgm

INNER JOIN dim_curso dc
    ON dc.nk_id_curso = c.pk_id_curso

INNER JOIN dim_unidade du
    ON du.nk_id_unidade = c.fk_id_unidade

INNER JOIN dim_tempo dt
    ON dt.dia = p.data_vencimento
